...
public function deleteAllScansByUser($id_user)
{
    // Iniciar una transacción para asegurar que todas las eliminaciones se completen correctamente
    $this->db->transStart();

    // Obtener todos los escaneos del usuario
    $scans = $this->db->table('scan')->where('id_user', $id_user)->get()->getResultArray();
    ...
    foreach ($scans as $scan) {
        ...
        $this->db->table('scan_details')->where('id_scan', $id_scan)->delete();

        if (isset($scan['id_network'])) {
            $this->db->table('network')->where('id_network', $scan['id_network'])->delete();
        }
